...
contract Freelancing is MasterSlave, Owned, Mortal {
    ...
    modifier open(uint _id) {
        require(projects[_id].status == Status.Open);
        _;
    }

    modifier done(uint _id) {
        require(projects[_id].status == Status.Done);
        _;
    }
    ...
    /// @param _stopped boolean state variable for stopping the contract
    ...
    function returnProject(uint _id)
        public
        view
        returns (
            string memory name,
